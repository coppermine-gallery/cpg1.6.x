/*
 Copyright (c) 2003-2020 Coppermine Dev Team
 @license    GNU General Public License version 3 or later; see LICENSE

 plugins/upload_h5a/js/upload.min.js
 @since  1.6.09
*/
var redirURL="",h5u_albSel=null,stagger=!1;function $id(f){return document.getElementById(f)}function $ae(f,h,k){f.addEventListener(h,k,!1)}function H5up_done(f,h){var k=".php?album="+h5u_albSel.value;redirURL=0<js_vars.user_id||1==js_vars.guest_edit?js_vars.site_url+"/editpics"+k+"&newer_than="+js_vars.timestamp:js_vars.site_url+"/thumbnails"+k;f&&$.post("notifyupload.php",{album:h5u_albSel.value});"1"==js_vars.autoedit&&0===h?window.location=redirURL:$id("gotoedit").style.display="table-row"}
...
function L(){q||(C=1,typeof("function"==H5up_done)&&H5up_done(F,G),G=F=0)}function l(b,a){b&&("ufo"==a&&F++,--r||g.length||L());if(!q&&g.length&&(!w||r<w)){var c=g.shift();new P(c);r++;v.innerHTML=g.length}0>=g.length&&(p.style.display="none",t.style.display="none")}function Q(b,a){var c=this;c.show=function(d){c.pb.style.backgroundPosition=Math.floor(c.pb.offsetWidth*d)+"px 0";1===d&&(c.pb.innerHTML=c.fObj.fileName,c.pb.className="indeterm")};c.msg=function(d,x){c.pb.innerHTML+="<br />"+d;x&&(c.pb.className=
...
a.actSize);"chnk"==a.upState&&a.chnkNum?(a.pBar.show((a.startByte+n)/a.size),y(n-a.lastsz),a.lastsz=n):"upld"==a.upState&&(a.pBar.show(e.loaded/e.total),n=Math.round(e.loaded/e.total*a.size),y(n-a.lastsz),a.lastsz=n)}},chng:function(e){4>this.readyState||(200!==this.status?(y(a.size-a.startByte-a.lastsz),a.lastsz=a.size,0===this.status?(a.pBar.msg("-- "+js_vars.h5uM.aborted,!0),a.doChnk?(a.upState="abrt",I()):d()):(a.pBar.msg(this.statusText||this.status,!0),d())):200===this.status&&(this.responseText.length?
a.pBar.msg(this.responseText,!0):a.doChnk?I():M()))},fail:function(e){a.pBar.msg(a.xhr.responseText,!0)}};a.abort=function(e){a.xhr?(e=a.xhr.readyState,4>e&&0!==e?a.xhr.abort():z.abrt()):(a.pBar.remove(),a.pBar=null)};a.pBar=new Q(a,a.doChnk?"chunked":"");var A="",N=b.name.split(".");1==N.length||"object"==typeof js_vars.allowed_file_types&&0>js_vars.allowed_file_types.indexOf(N.pop().toLowerCase())?A='<img src="'+js_vars.H5uPath+'css/info.png" class="infog" onclick="showAllowedExts();" /> '+js_vars.h5uM.type_err:
b.size>js_vars.maxfilesize&&(A=js_vars.h5uM.size_err);if(A)a.pBar.msg(A,!0),y(b.size),a.xhr=null,l(!0,"errM");else return a.xhr.onreadystatechange=z.chng,a.xhr.upload.onerror=z.fail,a.xhr.upload.onprogress=z.prog,a.doChnk?I():M(),this}var u,K,H,v,g=[],w=js_vars.concurrent,q=!1,r=0,D=0,E=0,C=0,F=0,G=0,p,t;f.H5uSetup=function(){var b=$id("upload_field"),a=$id("dropArea");f.File&&f.FileList?(v=$id("qcount"),p=$id("qstop"),t=$id("qgocan"),b&&($ae(b,"click",O),$ae(b,"change",J)),b=new XMLHttpRequest,b.upload&&
($ae(a,"dragover",B),$ae(a,"dragleave",B),$ae(a,"drop",J),a.style.display="block",u=$id("totprogress"),H=$id("fprogress")),h5u_albSel=document.getElementsByName("h5u_album")[0]):($id("h5upldrow").style.display="none",$id("navailrow").style.display="table-row")};f.H5uQctrl={stop:function(){q=!0;p.style.display="none";t.style.display="inline-block"},go:function(){q=!1;p.style.display="inline-block";t.style.display="none";if(stagger)l(!1,"go");else for(;g.length&&r<w;)l(!1,"go")},cancel:function(){g.length=
0;q=!1;t.style.display="none";v.innerHTML=0;r||L()}}})(window,"uniload.php","autorient flistitl title caption keywords user1 user2 user3 user4".split(" "));function showAllowedExts(){alert(js_vars.h5uM.extallow+js_vars.allowed_file_types.join(", "))}function shide_titlrow(f){var h=$id("titlrow");f.checked?h.style.display="none":h.style.display="table-row"}$(document).ready(function(){H5uSetup()});