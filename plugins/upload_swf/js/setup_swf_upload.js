/*************************
  ...
  ********************************************
  ...
  $HeadURL: http://svn.code.sf.net/p/coppermine/code/trunk/cpg1.6.x/js/setup_swf_upload.js $
  $Revision: 8530 $
**********************************************/
...
SWFUpload.onload = function () {
    var settings = {
        ...
        button_disabled_text: '<span id="browse_button" class="browse">--' + js_vars.lang_upload_swf_php.browse + '--</span>',
    ...
    $("select[name='swf_album']").change(function() {
       ...
       if ($(this).val()) {
       		//swfu.setButtonTextStyle('.browse { color: #FF6666; }');
           swfu.setButtonDisabled(false);
           // Set the button style to enabled
        //   alert(button_enabled_style);
           swfu.setButtonTextStyle(button_enabled_style);
           swfu.setButtonText(settings.button_text);
       } else {
       		//swfu.setButtonTextStyle('.browse { color: #6666FF; }');
           // If no album is selected then disable the browse button
           swfu.setButtonDisabled(true);
           // Set the button style to disabled
        //   alert(button_disabled_style);
           swfu.setButtonTextStyle(button_disabled_style);
           swfu.setButtonText(settings.button_disabled_text);
       }
    });
}
