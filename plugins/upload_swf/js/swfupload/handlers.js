/*************************
  ...
  ********************************************
  ...
  $HeadURL: http://svn.code.sf.net/p/coppermine/code/trunk/cpg1.6.x/js/swfupload/handlers.js $
  $Revision: 8540 $
**********************************************/
...
function uploadSuccess(file, serverData) {
    try {
        ...
        if (1 * $('#upload_count').text() > 0) {
            ...
            var listbox_selected_value = $("select[name='swf_album']").val();
            var listbox_selected_text = $("select[name='swf_album'] :selected").text();
            $("select[name='swf_album']").children().remove().end().append('<option value="' + listbox_selected_value + '" selected="selected">' + listbox_selected_text + '</option>');
        }
    } catch (ex) {
        this.debug(ex);
    }
    ...
    if (this.getStats().files_queued === 0) {
        ...
        if (notify_upload) {
            $.post('notifyupload.php', {
                album: $("select[name='swf_album']").val()
            });
