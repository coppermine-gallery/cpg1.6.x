...
error_reporting(-1);
//file_put_contents('plugins/html5upload/chunklog.txt', "\nSCF ".print_r($superCage->files->getInt('/userpicture/error'), true), FILE_APPEND);

$h5u_debug = !!($CONFIG['debug_mode']==1 || ($CONFIG['debug_mode']==2 && GALLERY_ADMIN_MODE));

$grpn = USER_ID > 0 ? (int) $USER_DATA['group_id'] : 0;
$grpc = $grpn ? $grpn : '';
$h5u_cfg = isset($CONFIG['html5upload_config'.$grpc]) ? unserialize($CONFIG['html5upload_config'.$grpc]) : unserialize($CONFIG['html5upload_config']);
...
//file_put_contents('chunklog.txt', "\ntok ".print_r($superCage->files->getInt('/userpicture/error'), true), FILE_APPEND);

// process any chunked upload actions
$is_a_chunk = false;
$matches = array();
if ($superCage->post->keyExists('chunkact')) {
	if ($superCage->files->getInt('/userpicture/error')) uni_exit();
...
$error_code = $superCage->files->getInt('/userpicture/error');
...
if (!$superCage->files->getRaw('/userpicture/name')) {
...
if (!is_uploaded_file($superCage->files->getRaw('/userpicture/tmp_name'))) {
...
if ($max_file_size && filesize($superCage->files->getRaw('/userpicture/tmp_name')) > $max_file_size) {
...
if ($is_a_chunk) {
	...
	if (!$chunkObj->addChunk($superCage->files->getRaw('/userpicture/tmp_name'), $superCage->post->getInt('chunkNumber'))) {
		uni_exit();
	}
...
if ($superCage->post->getInt('use4titl')) {
