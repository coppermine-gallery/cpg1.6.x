...
class cpg_PHPMailer
{
	...
	protected function smtpSend($header, $body)
	{
		...
		if (count($bad_rcpt) > 0) {
			...
			throw new cpg_phpmailerException(
				...
				try {
					...
					if ($tls) {
						if (!$this->smtp->startTLS()) {
							throw new cpg_phpmailerException($this->lang('connect_host'));
						}
	...
	public function setLanguage($langcode = 'en', $lang_path = '')
	{
		...
		if (!$this->language) $this->language = $PHPMAILER_LANG;
		return true;
	}
		...
		if ($this->isError()) {
			...
			try {
				...
				if (false === file_put_contents($file, $body)) {
					throw new cpg_phpmailerException($this->lang('signing') . ' Could not write temp file');
				}
