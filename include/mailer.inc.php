...
class cpg_PHPMailer
{
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new cpg_phpmailerException($error_message);
            }
    ...
    public function send()
    {
        ...
        } catch (cpg_phpmailerException $exc) {
            $this->mailHeader = '';
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new cpg_phpmailerException($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty && empty($this->Body)) {
                throw new cpg_phpmailerException($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new cpg_phpmailerException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new cpg_phpmailerException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new cpg_phpmailerException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new cpg_phpmailerException($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) && !$this->smtp->data($header . $body)) {
            throw new cpg_phpmailerException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            throw new cpg_phpmailerException($this->lang('recipients_failed') . $errstr, self::STOP_CONTINUE);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (static::ENCRYPTION_STARTTLS === $secure || static::ENCRYPTION_SMTPS === $secure) {
                ...
                if (!$sslext) {
                    throw new cpg_phpmailerException($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[2];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            ...
                            throw new cpg_phpmailerException($message);
                        }
                    ...
                    if (
                        ...
                        throw new cpg_phpmailerException($this->lang('authenticate'));
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new cpg_phpmailerException($this->lang('extension_missing') . 'openssl');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new cpg_phpmailerException($this->lang('signing') . openssl_error_string());
                }
            } catch (cpg_phpmailerException $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment(
        ...
        try {
            if (!static::fileIsAccessible($path)) {
                throw new cpg_phpmailerException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
            ...
            if (!$this->validateEncoding($encoding)) {
                throw new cpg_phpmailerException($this->lang('encoding') . $encoding);
            }
        ...
        foreach ($this->CustomHeader as $k => $pair) {
            if ($pair[0] == $name) {
                ...
                if (strpbrk($name . $value, "\r\n") !== false) {
                    if ($this->exceptions) {
                        throw new cpg_phpmailerException($this->lang('invalid_header'));
                    }
 ...
 */
class cpg_phpmailerException extends \Exception
{
    /**
